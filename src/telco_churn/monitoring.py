...
def create_or_refresh_monitoring(
    ...
    parses the request and response JSON fields,
    writes the resulting DataFrame to a Delta table for monitoring purposes.
    ...
    df_final_with_status = (
        ...
        .withColumn(
            "prediction",
            F.when(
                F.col("churn_probability").isNotNull(),
                (F.col("churn_probability") > 0.5).cast("int"),
            ).otherwise(F.col("raw_prediction").cast("int")),
        )
    )
    ...
    df_final_with_status = df_final_with_status.withColumn(
        "MonthlyCharges", F.col("MonthlyCharges").cast("double")
    )
    df_final_with_status = df_final_with_status.withColumn(
        "TotalCharges", F.col("TotalCharges").cast("double")
    )
